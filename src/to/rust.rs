...
impl ToWriter for RustWriter {
    ...
    fn write(&mut self, writer: &mut BufWriter<File>, interface: &Interface) {
        ...
        write!();
        write!("#[no_mangle]");
        write!("pub extern \"C\" fn Alloc_FFI(length :int32) -> *mut u8");
        write!("{");
        write!("unsafe");
        write!("{");
        write!("let layout = Layout::from_size_align_unchecked(length as usize, 0);");
        write!("return System.alloc(layout);");
        write!("}");
        write!("}");

        write!();
        write!("#[no_mangle]");
        write!("pub extern \"C\" fn Free_FFI(ptr: *mut u8, length :int32)");
        write!("{");
        write!("unsafe");
        write!("{");
        write!("let layout = Layout::from_size_align_unchecked(length as usize, 0);");
        write!("System.dealloc(ptr, layout);");
        write!("}");
        write!("}");
