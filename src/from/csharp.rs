        ...
        {
            {
                ...
                let parameters_str = parameters
                    .into_iter()
                    .map(|p| format!("{}{} {}", p.r#type, cc!(interface.is_type_blittable(interface.get_type(&p.r#type)) => "", "_FFI"), p.name))
                    .collect::<Vec<String>>()
                    .join(", ");

                let return_type_name = cc!(method.returns.len() == 0 => "void".to_string(), method.returns[0].r#type.clone());
                let ffi_suffix = cc!(interface.is_type_blittable(interface.get_type(&return_type_name)) => "", "_FFI").to_string();

                write!();
                write!("[DllImport(LIBRARY_NAME)]");
                write!("private static {}{} {}_FFI({})", return_type_name, ffi_suffix, method.name, parameters_str);
                write!("{");
                write!("}");
            }
